...
| ⏸️ | **10 works still have no cover — and the reason is a gap in the ladder, not the books** | **57 → 10** overnight (stranded 12, Google Books 20, LLM 18). ⚠️ The interesting failures are *The Sea of Monsters*, *The Nightmare Before Christmas* and *The Day the Crayons Made Friends* — famous books that any cover database holds. They fail because **every free rung is keyed on ISBN**, and these works have editions with no ISBN (the Percy Jackson set was hand-created without one). **The fix is a free title+author Open Library search rung**, not more paid lookups — `packages/isbn/src/search.ts` already builds a cover URL from `cover_i` and nothing calls it for this purpose. Re-running `--llm` on the same input is not worth it; it already tried these. | Claude — build the title-search rung |
