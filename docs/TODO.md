# TODO â€” library_catalog (ACTIVE work log)
...
> finished work â€” larger than the 1,688-line file that caused the rule to be
...
> | Gotchas | [`info/gotchas.md`](info/gotchas.md) | The traps that cost real time â€” findable by symptom, not by the day they bit. |
> | Decisions | [`info/decisions.md`](info/decisions.md) | Rationale for calls made, including the ones argued *against*. |
> | Rollback ids | [`access/rollback-points.md`](access/rollback-points.md) | Operational â€” the 3am "put it back" reference. |
>
> âš ï¸ Sections were moved **whole â€” cut and paste, never summarised**. The
> summary always drops the *why*, and the why is the only reason to keep
> history at all.
>
> âš ï¸ This split does **not** reinstate the problem that consolidating several
> competing *living* docs solved. An archive and a topic reference are not
> living docs â€” they do not compete with this file for "what is happening
...
column nothing wrote): a book can be marked done for a question nobody put to
it.
...
## ðŸ§° Tech debt (owner-ordered section, 2026-08-17: "all tech debt stuff move
## in to there so we can handle tech debt stuff later")

Nothing here is urgent, broken, or user-visible; each is a deliberate
"later". An item leaves by being fixed (â†’ DONE.md) or by being promoted to
real work. New debt goes HERE, never scattered.

- **Revoke the stale broad Cloudflare user token** â€” "Edit Cloudflare
  ...
  2" (Aug 17) IS the live CI-deploy token â€” keep, but it's broader than CI
  needs; narrowing = mint scoped replacement + `gh secret set` Ã—3 repos.
- **Let the donor donate the PRINTED volume number too** â€” `routes/donor.ts`
  ...
  already close â€” quality, not convergence.
- **audiobook `scripts/` not linted in CI** â€” the lint workflow covers `app
...
- **The cp1252 âš ï¸-in-print class** â€” three incidents in two days (smoke
...
- **CSP `frame-src` asymmetry on apex** â€” `/universes` + `/status` don't
...
- **`dl_ebooks` column is dead but standing** â€” deprecated-unused by
...
- **The TBR legacy display-name fallback** âš ï¸ **UPDATED 2026-08-18: the prod
  migration is APPLIED (181/181) and the owner has DECIDED the last 53** â€”
  ...
  carry, **0 duplicates**) but âš ï¸ **the run is blocked by the operating
  ...
  migration, `info/tbr.md` Â§8). `legacyReadingListDocId`, the `legacyDocId`
  ...
  a retired v1 passphrase account with no Firebase uid. âš ï¸ **REMOVAL
  ...
  whether the owner wants them reassigned or deleted â€” **an owner decision,
  not a cleanup.**

## ðŸ”¥ Owner asks 2026-08-16 late evening â€” status board

### Third wave (2026-08-17 morning)
- **ðŸ¤– GABI, the conversational fixer â€” PHASE 0 SHIPPED; DISCORD IS NEXT.**
  Phase 0 (read-only) is built, deployed to both instances and archived whole in
  [`DONE.md`](DONE.md); the living record is
  [`info/gabi-fixer-design.md`](info/gabi-fixer-design.md) (Â§9 = what is and is
  not built, Â§13 = the file map, Â§7.4 = measured costs). What is still moving:
  - âš ï¸ **HER FIRST CONVERSATION NEEDS HER EYES.** The panel is live on
    `padhard.heygabi.ai` (top bar, speech-bubble icon beside the search and the
    cog) and the posture and route were verified from outside â€” but every
  ...
  - âš ï¸ **THE MEMORY'S ACCEPTANCE TEST IS THE SAME SHAPE, AND ALSO UNRUN.**
    Panel v2 shipped 2026-08-18 to both instances â€” the panel now uses GABI's
    ...
    **nobody has held a real conversation, closed the tab, come back inside
    ...
    1. <https://library.heygabi.ai> (or `padhard.heygabi.ai`) â†’ speech bubble â†’
    ...
    3. Expected: she answers from the earlier exchange, and the panel shows
       *"Picking up where you left off â€” GABI still has the last 2 things said
       hereâ€¦"* above the answer.
    4. Wait past 30 minutes and repeat step 2. Expected: she does **not**
       remember, and the line does not appear.
    âš ï¸ Step 4 is the half that is easy to skip and the one that proves the
    privacy posture rather than the feature.
  - **ðŸ’¬ Discord DM is the NEXT phase** (owner, 2026-08-17: *"we can do
    discord right after"*) â€” promoted ahead of the write phases. Two of the
    three parts already exist and are front-end-agnostic (`GABI_TOOLS`, the turn
    route); what a Discord surface must write is its own EXECUTOR. âš ï¸ Design
    Â§10.2's four blockers are unchanged and none was solved by phase 0 â€” start at
    shape **(b)**, propose-and-deep-link, which needs none of them.
  - **Then phases 1â€“3** â€” the write slice (`research_book`, blank-only
    `set_book_details`, `undo_changes`), covers, small batches. âš ï¸ Each needs
    what phase 0 deliberately does NOT have: the confirm lane (Â§6), the
    provenance stamp (Â§5.2, `changed_how` + a `gabi:<conversationId>` note), and
    the manifest UI. All the policy is settled (Â§12, answered) â€” none of it is
    implemented.
  - ðŸ§‘ **Owner action, the one open Â§12 row:** confirm Samantha's Anthropic key
    ...
    debt list above. Everything else in Â§12 is answered.
    - âš ï¸ **That cap is not hypothetical â€” IT HAS ALREADY FIRED.** Her key hit
      ...
      **allowance itself is untouched**, so lookups on her instance stay dead
      until 1 September unless someone raises it. Evidence the backstop works â€”
      and a decision the owner has to make, not a bug to fix.

### ðŸ‘ï¸ Needs a signed-in eye â€” padhard's Missing/queue FAILED rows
...
1 September 2026â€¦"* rather than a JSON body. Say-what-you-know and Look-again
should still be beside them.
- **âœ… EBOOK DOWNLOAD IS NOW A ROLE, NOT A CHECKBOX (owner, 2026-08-17: *"For
  ebooks I don't want a download check box, I want to use roles we have. Set up
  the roles to match library."*)** â€” built in `catalog-platform`
  ...
  **Why it appears on this board at all:** the pattern the owner names by
  ...
  - **What arrived here:** the `downloadEbooks` field left `SeenAnswer` /
    ...
    to `{status, visibility, checkedAt}` â€” the form they had before yesterday
    taught them the field. âš ï¸ That is a ROUND TRIP, not a stale test; the file
    header explains it so nobody "fixes" it back.
  - **The grant now:** promote on the admin page's Audiobook role dropdown
    (`download` floors at `admin`). `vis_ebooks` â€” seeing the shelf and reading
    in the viewer â€” is UNCHANGED.
- **ðŸ“± Ebook reader PWA/offline â€” ON THE TABLE FOR LATER (owner, 2026-08-17:
  "Add pwa back on the table for later"):** not in viewer v1. âš ï¸ The design
  ...
  `download` capability â€” **admin floor since 2026-08-17, and NOT the
  per-person `dl_ebooks` checkbox that briefly existed** â€” never bundled free
  with reading. Whoever builds it later starts from that sentence.
- **ðŸ”´ EBOOK GATE â€” the three things it left open (built 2026-08-17, whole
  record in [`DONE.md`](DONE.md)):**
  1. **Purge the edge cache.** `audiobooks.heygabi.ai/ebooks.json` was still
     served from Cloudflare's edge AFTER the deploy that stripped it â€”
     ...
     **Owner:** Cloudflare dashboard â†’ `heygabi.ai` zone â†’ Caching â†’
     Configuration â†’ Purge Custom URL, for `/ebooks.json` and
  ...
  2. **The prod promote.** `ebooks.heygabi.ai` proxies the PROD branch, whose
     `site/ebooks.html` is still the pre-gate page â€” so the SHIM is not live
  ...
  3. ~~**Ebook rows leave the estate index at the next CI deploy.**~~
     âœ… **CLOSED 2026-08-17 â€” the owner chose option A** (move the push out of
     ...
     âš ï¸ **Two things to carry forward for this repo's own pushes:** never add
     ...
     working â€” check `/api/health` counts or search as a member instead.
Landed and archived â€” the TBR instant-clear built in `audiobook_catalog`
...
- **Sequencing (owner):** deliver current batch â†’ **Discord portal** (owner is
  nearly home for the owner-present steps) â†’ **EPUB/PDF viewer** after.
...
- **Donor reciprocity flip** (open thread from the shipped donor sweep,
  ...
  line in the main `[vars]` â€” `DONOR_URL = "https://padhard.heygabi.ai"` â€”
...
- **EPUB/PDF in-browser reader** (owner ask 2026-08-16: *"how hard would it be
  ...
  book on the site?"*; sequenced after the Discord portal) â€”
  âœ… **DESIGN DONE 2026-08-17. PHASE 0a (bucket + file ingest) BUILT the same
  day** â€” see [Â§2.2a](info/ebook-viewer-design.md) for the evidence and
  `audiobook_catalog/docs/info/ebooks-r2-ingest.md` for the reference.
  ðŸ“„ **[`info/ebook-viewer-design.md`](info/ebook-viewer-design.md)** â€” full
  ...
  **From phase 1a on, nothing is built: no worker route, no reader page, no
  ...
  - ðŸ”´ **OWNER ACTION carried over from phase 0a: one book is not in the
    bucket.** âš ï¸ **Measured: `wrangler r2 object put` refuses files over
    300 MiB** (`--pipe` too â€” it is the Cloudflare REST endpoint's ceiling, not
    ...
    an **R2 API token** wrangler cannot mint (dashboard â†’ R2 â†’ Manage R2 API
    Tokens, Object Read & Write on `estate-ebooks`). âš ï¸ **Check the duplicate
    question first** (Â§10): if the 143 MiB `whitesand.epub` is the same graphic
    novel, deleting the omnibus removes the only file that needs the token.
  - ðŸ”´ **Pipeline wiring is a one-line conductor step, deliberately left
    undone** â€” `sync_to_drive.py` auto-runs 3Ã—/day and was contested. It is
  ...
  **What the design changed about the first take** (which said "epubs off R2
  ...
  - âš ï¸ **The files are NOT in R2.** Measured: only covers are (1,850 + 83
    ...
    framing did not price** â€” **1.805 GB / 1.681 GiB total** (138 EPUB =
    1.084 GiB, 30 PDF = 0.598 GiB), against R2's 10 GB free tier.
  - ~~âš ï¸ **Range requests are a PDF technique, not an EPUB one.** An EPUB is a
    ...
    ðŸ”¬ **FALSIFIED BY MEASUREMENT 2026-08-17 â†’
    ...
    `GET` each) at ~3Ã— file size in JS heap â€” **1,207 MB for the 393 MiB
    ...
    so ðŸ”´ owner decision #1 below re-opens with new evidence.
  - âœ… The auth-gating call was right, and it is smaller than feared: the
  ...
  - âš ï¸ **It must NOT gate on Â§4.5 visibility** â€” `vis_audiobook` is the estate's
  ...
  - Reading position becomes the **first `uid`-keyed collection in this
    estate** â€” the reader's readership is token-bearing by construction, unlike
  ...
  **Recommended phase 1** (after phase 0, the bucket + ingest step): the gated
  ...
  ðŸ”´ **Owner decisions before phase 1** (Â§11 of the doc, 8 of them; the two
  that matter most): **PDF first or EPUB first?** and **what "preview" means â€”
  ungated first chapter, or members-only read with a richer card + PDF
  first-page thumbnail?** (~~recommended: PDF first~~ â€” **the PDF-first
  recommendation is now unsupported**, see the measurement above; members-only
  still recommended).

## ðŸ“š Ebook split â€” âš ï¸ PHASE 5 (retire ingest + prune) IS STILL OWED

**Decided and mostly built.** The insight below (owner, 2026-08-16) became
`catalog-platform/docs/info/ebook-split-design.md`; phases 1â€“4 have shipped and
ebooks live at **ebooks.heygabi.ai**. What is left in *this* repo is phase 5.

**Phase 5, unchanged from the design's Â§6 table** â€” stop running
...
exceeds the 20% guard **by design** â€” the ceremony is the point); delete the
ebook-only works. âš ï¸ **Re-measure `user_book` for `'human'`-asserted read states
on those works before deleting â€” it must be 0**, and it was 0 at design time
only because nobody had typed one in yet.

**Measured 2026-08-18 04:55Z, so phase 5 knows what it is deleting:** 387 works,
**94 ebook-only**, 127 ebook editions. âš ï¸ The two ebook figures have not moved
since the design measured them on 2026-08-16 while the catalog grew by 25 works
in twenty minutes â€” the ebook rows are a closed 2026-08-09 import with no
producer still pointed here. If they ever *grow*, the ingest is back on.

**What landed instead, 2026-08-18 â€” the display half, on the owner's ask**
...
is the physical shelf. **No data was deleted** â€” that is phase 5's job, and the
...
âš ï¸ **Still showing all 387 works, and deliberately so â€” an owner decision, not
an oversight:** the collection grid, the `Format: Ebook (94â†’126)` facet and the
...
(`ebookOnly` defaulted on in `collectionQueryFrom`) â€” ask him, do not assume.**
...
|---|---|---|
| Audiobooks | âœ… already its own site | |
| **Ebooks** | âœ… **behaves like audiobooks** | |
| Physical books | | âœ… a specific copy on a specific shelf |
| Board games | | âœ… (a physical copy, though played together) |

Ebooks currently live INSIDE the physical library catalog â€” `site/ebooks.json`
...
âš ï¸ **This is exactly the question the second-household federation runs into.**
"See who owns what" is meaningful for physical books and games, and close to
meaningless for ebooks and audiobooks â€” those are "do we have it", not "whose
...
~~**Not a build yet.** Open questionsâ€¦~~ â€” **all four were answered on
2026-08-16** and the answers are the design doc's Â§2â€“Â§5, kept there rather than
...
pruned** (Q2 â€” the phase 5 above); the shelf server needs one runbook line
...
## ðŸ¤ A second household's library, federated with ours (owner ask 2026-08-16)

**Deferred by the owner the same day it was raised â€” "do the next catalog
later" â€” recorded now so it is not lost.**
...
1. âš ï¸ **She is less technical.** Every operational assumption this estate rests
   on â€” a pipeline on a home machine, wrangler from a laptop, reading a runbook
   â€” is unavailable. Whatever is built has to run without her ever seeing a
   terminal.
2. âš ï¸ **She is not local.** No shared LAN, no "I'll set it up on your machine",
...
3. **The point is the JOIN, not the copy.** "See who owns what" means the two
   catalogs must be comparable â€” which is what the shared index
   (`index.heygabi.ai`) already does across our three catalogs, and is the
   obvious foundation rather than a new mechanism.

âš ï¸ **Do not start this by cloning a repo.** The interesting design question is
...
## ðŸ“¸ Owner note â€” Illumicrate edition photos (2026-08-14) â€” âš ï¸ THE DASHBOARD NAG IS GONE, THIS NOTE IS NOW THE ONLY REMINDER

> **2026-08-18 (~14:15 Phoenix), owner order: "yes remove the need cover but
> keep it in our todolist."** Works 224â€“228 had `cover_status = 'standin'`
> cleared to NULL (art unchanged, 5 change_log rows, batch
> `illumicrate-standin-clear-20260818`) so `/?needs=cover` measures **0**.
> The stand-in flag was the reminder mechanism â€” with it gone, THIS section
> is the reminder of record. When the photos are taken: each work page â†’
> Cover panel â†’ upload; every prior cover stays selectable in the
...
edition/cover images â€” the audiobook covers now being pulled are the standard
...
âš ï¸ **These five works (224â€“228) are now the ENTIRE `/?needs=cover` list.**
...
box-set split's coverless offspring all filled â€” see [`DONE.md`](DONE.md).

âš ï¸ **No backfill script can close this one, and re-running one is wasted
...
verified-loading URL (59â€“73 KB each, re-checked 2026-08-18) and are on the
...
| ðŸ”¨ | In flight |
| â¸ï¸ | Blocked â€” the blocker is named |
| ðŸ’¤ | Deliberately deferred |
...
Movement since the crowdfunding rescan landed: works 233 â†’ 258, owned copies
117 â†’ 152. Audio corroboration: **17 series confident, 2 hedged** â€” and the 2 are
...
### ðŸ”¨ Completionist Chronicles 12 & 13 â€” signed, in BOTH formats, 2026-08-13
...
âš ï¸ **Both works already exist and both are ebook-only**, so this is four copies to
...
Both Dakota Krout / **Mountaindale Press**. No ISBNs â€” the photographs are front
covers and back blurbs, no barcodes â€” so the editions go in without one and a
barcode scan can fill them later.

âš ï¸ **`copy.is_signed` is the whole point of this entry.** It is one of the fields
`docs/info/crowdfunding-and-accessories.md` says *only a person can fill* â€” no
importer or lookup will ever set it â€” so if these four copies are recorded without
...
âš ï¸ Note the rest of the series for contrast: works **238â€“242** (*Ritualist*â€¦
*Ruthless*) each hold a hardcover with **`signed = 0`**.

**âœ… And that question is now partly answered â€” #238 *Ritualist*'s hardcover IS
...
â€” #238 already holds the hardcover, and its existing copy needs `is_signed`
flipped to true.

â³ **Still unknown for 239â€“242** (*Regicide*, *Rexus: Side Quest*, *Raze*,
*Ruthless*) â€” all four hold a hardcover with `signed = 0`. If the whole
Kickstarter set is signed then all four are wrong, but âš ï¸ **do not infer it from
#238** â€” one confirmed book is not evidence about four others, and this is exactly
...
### âš ï¸ THE SCANNING IS NOT FINISHED â€” more books arrive tomorrow

Owner, on going to bed 2026-08-13: *"be aware we're not done scanning books, just
done for the night, more will come tomorrow."*

**This reprioritises the overnight list.** The catalog went 258 â†’ 342 works in one
...
1. **Intake-path fixes** â€” the edition picker, a format question at intake (a board
   book still lands as `paperback` through `AddWork`, the scan path and any
   importer â€” see the standing rule in `docs/info/series-formats-and-audiobooks.md`),
   and anything that stops the Open Library **work-level aggregate** bug recurring.
   âš ï¸ That last one corrupted three works tonight and *will* fire again on the next
   series whose OL record is filed that way.
2. **Cover swap** â€” 147 covers are third-party hotlinks and every new book adds more.
3. **One-off data cleanup** â€” last, and never at the cost of 1 or 2.

âš ï¸ **Do not treat the catalog as final.** Anything that assumes a fixed row count, a
complete series, or an empty queue will be wrong by tomorrow afternoon. The
`/queue` residue in particular will grow again â€” an empty queue tonight is not a
finished job.

---

### âš ï¸âš ï¸ THREE BUGS found while adding books by hand â€” 2026-08-13, two now FIXED
...
**1. âš ï¸ The typed ISBN is NOT stored. `/add?mode=type` has an ISBN box, and
nothing persists it.** Measured: works **265, 266, 267, 269, 274** all have
`editions = 0`, so no ISBN, no publisher, no year â€” for books whose ISBN was typed
in or scanned. Compare the owner's own adds (#268, #270â€“273, #275, #276), which
...
that need hand-entry â€” the ones no service knows â€” **the one hard identifier the
...
**2. âš ï¸ "AND WEâ€¦" defaults to "just catalogue it â€” record no copy".** For a
scanning session â€” where every book is physically in your hands â€” the default is
...
"want it â€” put it on the wishlist". Default should almost certainly be "have it"
when reached from a scan.

**3. âš ï¸ A save can fail silently, and the cleared form looks like success.**
*My First Farm Animals* (9781839035920) was typed, saved, and the form went blank
â€” and **no row exists**. Cause is almost certainly hydration: the fields were
...
| 269 | Who Goes Roar? | ISBN 9781836422808 **and a copy â€” it is owned** |
| 274 | My First Toys | ISBN 9781839035944 |

âš ï¸ Also: #269 was entered as author **"Make Believe Ideas"** while the catalog
already holds #144 *Never Touch a Dinosaur!* as **"Make Believe Ideas  Ltd."**
(with a double space). Two author spellings for one publisher â€” pick one.

---

### ðŸ”¨ Books whose ISBN resolves to NOTHING â€” transcribed from photographs, 2026-08-13

âš ï¸ **Growing list, one root cause.** The owner is scanning and these keep landing:
the ISBN is real and printed on the book, and **every rung of the ladder returns
no answer at all** â€” so the scan row has no title, no author, and `isAddable`
...
**"Who Goes Roar?"** â€” a tabbed board book of dinosaur sounds
| field | value |
|---|---|
| ISBN13 / ISBN10 | **9781836422808** / 1-83642-280-6 |
| publisher | **Make Believe Ideas** (make believe ideas ltd, Berkhamsted, Herts UK Â· 557 Broadway, New York) |
| copyright | 2019 Â· $4.99 US / $6.99 CAN |
| illustrator | **Shannon Hays** â€” the ONLY credit on the book |
| author | âš ï¸ none named. Publisher-as-author, per the catalog's convention (*Scholastic* #141, *Bendon* #137) |

**í•˜ì¸„í•‘ì˜ ëˆˆë¬¼** â€” Korean *Catch! Teenieping* tie-in
| field | value |
|---|---|
| ISBN | **9791165384678** (979-11-6538-467-8) Â· 15,000ì› |
| franchise | ìºì¹˜! í‹°ë‹ˆí•‘ (Catch! Teenieping) |
| series badge on cover | **í•˜ì¸„í•‘ ë§ˆìŒ ë™í™” 2** â€” so volume **2** of that series |
| credits | ì›ìž‘Â·ê·¸ë¦¼ **SAMG** Â· ì—®ìŒ **ì•„ì´íœ´ë¨¼ íŽ¸ì§‘ë¶€** (compiled by the iHuman editorial dept) |
| publisher | **ì•„ì´íœ´ë¨¼ / iHuman** |

âš ï¸ **Open question, and it decides how these two file together:** the catalog
already holds **#195** â€” *ìŠˆíŒ… ìŠ¤íƒ€ ìºì¹˜! í‹°ë‹ˆí•‘: ì•½ì† ì˜ ì˜¤ë¡œë¼í•‘*, ISBN
9791165384548 â€” under series **ë§ˆìŒì„ ì±„ìš°ëŠ” ë™í™”**. The new book's cover says
**í•˜ì¸„í•‘ ë§ˆìŒ ë™í™”**. Are those the same series or two different ones? Filing them
...
âš ï¸ Also note **#195 is on the cover-photo pull list** â€” do not confuse the two
Korean books, they are different ISBNs (â€¦4548 vs â€¦4678).

---

### ðŸ”¨ The Autumn Publishing 6-book set â€” cannot be scanned in, 2026-08-13

âš ï¸ **Read the back covers: "Sold as part of a set, not for resale."** These are
set-internal ISBNs. No retailer lists them individually, so **every rung of the
ISBN ladder returns nothing** â€” not a wrong answer, no answer. The scan row gets
...
**They must be typed in** â€” `/add?mode=type` ("Type a title"), which needs no
...
books with polyurethane-foam covers, series line on the covers is **My First** â€”
which is already a series in this catalog.

âš ï¸ **Author: `Autumn Publishing`.** No person is credited anywhere on these books.
That is the catalog's existing convention for publisher-branded board books â€”
*Scholastic* on #141, *Bendon* on #137 â€” not a placeholder.

---

### âš ï¸ #174 *I Love You, Little Bear* â€” the author on file is probably another book's

Owner: *"the publishing company editorial stuff wrote it togetherâ€¦ use the
publisher as the author."* They are right, and it is worse than a blank.

Researched 2026-08-13: **at least four different books share this exact title** â€”
Claire Freedman's, Angela Navarra's, a Scholastic edition, a Phoenix
International edition â€” and **none of them carries our ISBN 9781472327314**. So
...
âš ï¸ **Blocked on the same guard as the title:** `WorkFields` deliberately cannot
reach `authors`, because `work_key` derives from it. #174 has **no reviews on
either site**, so the key move is harmless here â€” which is exactly the
"gated on no-review-join" design in the item below.

---

### â¸ï¸ Edit any detail, an audit log, and adding a book with no author â€” 2026-08-13
...
typing an author â€” *There's a Mouse About the House!*, *Don't Tickle the
...
#### âš ï¸ The constraints, all measured â€” read before designing
...
| **`work_key` contains the author on purpose** | `0001_init.sql:99` â€” *"Title-only keys collide across authors constantly"* | a title-only key for authorless rows is a known-bad idea in this schema, not a shortcut |
...
| `WorkFields` **deliberately** cannot reach `title`/`authors` | same header | this is the guard, not an oversight â€” do not simply remove it |

âš ï¸ **The trap:** "flag for remediation" means the author gets filled in *later*,
which moves `work_key` â€” the exact thing the guard exists to prevent. It is
...
2. **Add with no author** â†’ row is created *and* a `work_watch` row is written in
   the same call, so it lands in `Needs â†’ To check` and cannot be silently
   forgotten. Migration 0040's rule: the flag travels with the write.
3. **An edit surface for everything else** â€” title included â€” gated on "this work
...
âš ï¸ **`audiobook_catalog` needs the same treatment** and shares the identity and
review store, so the audit-log table and the `work_key`-move rules should be
designed once, across both â€” see `catalog-platform` / `PLATFORM.md` Â§2.2 on what
...
as the author â€” *Scholastic* on #141, *Bendon* on #137 â€” so that is a legitimate
...
## Blocked â€” the live list

| | Item | Blocker | Who clears it |
|---|---|---|---|
| â¸ï¸ | **BackerKit `nbaslamking@gmail.com` is not signed in** | The browser holds `aim.com`, which has only *Words of Radiance*. The gmail account has at least the DCC Croc Box and probably more. âš ï¸ **Never sign in on the user's behalf.** | User â€” said "we will do nbaslamking@gmail.com next" |
| â¸ï¸ | **Four universe calls held for verification** | Will Wight (Cradle, The Last Horizon) â€” the author has *hinted* at a multiverse and nothing is established; Turncoat's Truth; Cultivating Chaos + The Axe Falls; Tailored Realities. All recorded in `_refused` in the shared list so they are not re-litigated. | User |
| â¸ï¸ | **Two series stay hedged at AUDIO?, and cannot be fixed by code** | *Arcane Pathfinder* (we hold 5, audio has 1â€“4) and *Legion* (we hold 1â€“2, audio has only the omnibus at rung 4). No volume is owned in both formats, so nothing can corroborate the numbering. âš ï¸ Loosening the matcher would turn a hedge into a lie. | User â€” a purchase, or nothing |
| ðŸ’¤ | **~100 physical books unscanned** | Standing backlog, **explicitly not a blocker**: *"Don't wait for books to be scanned to move on."* A book missing from the catalog usually means unscanned, ranked above "not owned" and well above "bug". | User, over time |
| âœ… | ~~**Percy Jackson covers: no per-book images exist**~~ | **Answered and built.** User: *"use the marketing image now but put a label on them."* Migration `0040` sets all five to the plain-background lineup and flags them `cover_status = 'standin'`, so they wear the picture **and** stay on the "Cover needed" list. âš ï¸ The five identical URLs are deliberate â€” nothing may dedupe them. Selected by `edition_name`, not by id. | Done |
| â¸ï¸ | **Two books claim contradictory series** | #213 *Secret Ingredient* records series "The Pengrooms"; #215 *Pengrooms* records "Pringle & Finn". Both by Paul Castle, both auto-filled, both sourced â€” and they cannot both be right. **Both now carry a `work_watch` row**, so they wear a **Check** mark and appear under `Needs â†’ To check`. The question is recorded; it still wants the user's eyes. | User â€” said they will verify later |
| â¸ï¸ | **Three books the model refused to identify** | #141 *Touch and Explore* (Scholastic), #160 *Bizzy Bear* (Nosy Crow), #174 *I love you, little bear* (Judi Abbot) â€” bare **series-line** titles, so a lookup returns the range rather than the book. Declining beat guessing. âš ï¸ **CORRECTED 2026-08-13: the â€œISBNs did not resolveâ€ premise was FALSE for #141 and #174** â€” the API resolves both; only *searching* the ISBN fails. **Re-running DOES help when it queries the API rather than the web**; a subtitle also helps â€” e.g. *Bizzy Bear: Fire Rescue*. | User, from the covers |
| â¸ï¸ | **4 works have no cover any rung can reach** | A Paw Patrol shaped board book, *Home Sweet Home*, a Korean Tinyping board book, *The Nightmare Before Christmas*. **There is now a way in**: the book page's Cover panel accepts a link to any image, verified before storing. Uploading a *file* additionally needs the R2 binding below. | User â€” paste four links |
| âœ… | ~~**Cover file upload needs an R2 binding**~~ | **Done 2026-08-11.** Bucket `library-covers` created, `COVERS` binding + `COVERS_BASE_URL` wired, deployed `0ab1e18e`. âš ï¸ The hostname is **`bookcovers.heygabi.ai`, not `covers.heygabi.ai`** â€” that one is already attached to the sibling's `audiobook-covers` bucket, and a custom domain belongs to exactly one bucket. Checked before choosing. Still worth adding a Cache Rule (`bookcovers.heygabi.ai/*` â†’ Edge TTL 1 year); safe because object names hash the file contents, so a replaced cover is a different URL. | Done |
| âœ… | ~~**Barnes & Noble was never imported**~~ | **Found 2026-08-11 while answering "what's labelled deluxe edition".** The scan was staged on 2026-08-10 and never imported, because the only importer was for pledges and rightly refused a shop order â€” so **zero of its 7 books were in production**. `scripts/import-shop-orders.mjs` is the missing half. Now: **3 owned, 4 preordered**. âš ï¸ This is also why the preorder tag had never rendered â€” it was correct all along with nothing to show. | Done |

### Answered by the user 2026-08-11

- **Percy Jackson set confirmed** â€” and independently verified: the group photo
...
- **Words of Radiance "+ Books" is solved.** The leatherbound shipped as **two
  ...
  edition delivered as two objects â€” not two different books, and not a mystery.
- **"DCC RPG + Unstoppable" â€” dropped.** The user says it is a D&D book, so the
...
- **The `/todo` page must NOT be public.** It stays built and pushed but
  ...
  has to move to a host that does â€” the catalog sites already sit behind
  Firebase sign-in.

### Cleared since the last revision

- ~~Main checkout dirty with unclaimed manager-role work~~ â€” it was the
  people/roles feature from earlier in the session; it committed itself as
  `a138019` + `c75d174`.
- ~~Deploy blocked by a dirty tree~~ â€” main is clean; five branches merged.
- ~~Barnes & Noble sign-in~~ â€” done, scanned, complete.
- ~~Kickstarter password verification~~ â€” user fixed; all 62 pledges enumerated.
- ~~Indiegogo sign-in~~ â€” done; only 3 pledges exist.
- ~~Worktrees typechecking against the main checkout~~ â€” no `node_modules` in a
...
## Crowdfunding rescan â€” 2026-08-11, IN PROGRESS
...
âš ï¸ **The list view paginates â€” there is a "Show more pledges" button and it must
...
### Result â€” **14 unrecorded book pledges**. COMPLETE. Report in the session scratchpad.

| account | coverage |
|---|---|
| Kickstarter | âœ… 61 successful + 1 active, after "Show more pledges" Ã—5 |
| Indiegogo | âœ… all 3 pledges; 2 are books |
| BackerKit `aim.com` | âœ… Pledges (1, no pagination) + Surveys p1+p2 + Active + Digital rewards |
| BackerKit `gmail.com` | âœ… Pledges (4) + Surveys Completed p1+p2 + Active p1+p2 + Digital rewards |

âš ï¸ **Two pagination traps.** Kickstarter renders 10 of 61 by default; **every**
...
**The finds:** Completionist Chronicles **1â€“5 in physical Grimoire editions**
(*Ritualist* box SHIPPED) Â· **Tamer 1â€“10 in paperback plus 11 on preorder** â€”
the book-7 tier was "WHOLE SERIES SIGNED PAPERBACKS", confirmed by the owner,
and no hardcover exists Â· **Beneath the Dragoneye Moons Complete Realmkeeper
Set** ($670, shipped) Â· *Worlds Beyond Number* graphic novel Â· *Monster Empire 2* Â·
*Ascend Online Book 1* Â· and from Indiegogo **Space Knight 5 and 6** â€” which is where the
existing unattributed Space Knight EPUBs came from. âš ï¸ The owner confirms print
copies exist and they own **Space Knight 1â€“9**, so that is 9 paperbacks, not 2.

**Tamer audio, settled by the owner 2026-08-12:**

| Volumes | Audio | Note |
|---|---|---|
| 2â€“6 | **not owned** | Confirmed correct â€” a real gap, not a filing error. Stop re-checking. |
| 11 | **owned** | âš ï¸ But **BookFunnel will not serve the download at this time**, so no m4b exists locally and the audiobook catalog cannot see it. |

âš ï¸ Tamer 11 is the case the schema handles badly: *owned but unobtainable*. It
is not a gap to chase and not a file the pipeline can ever acquire â€” any audit
...
âš ï¸ **Nothing has been written to the database.** The owner asked to verify first.
...
in the Words of Radiance tier â€” that order detail refused to open twice.

---

## Staged, waiting on the user to run â€” 2026-08-10
...
Run **in this order** â€” the first is free and the second deliberately skips what
the first fixes.
...
âš ï¸ **Step 4 rewrites `audiobook_holding`.** It marks the five wrong Tamer rows
and the wrong Primal Hunter row `stale_at` rather than deleting them (migration
0003's rule), and writes the correct rows for Tamer 7â€“10. Expect **six fewer
false claims** and the honest total to land near 45.

### Optional and **paid** â€” not run, gate separately

```bash
# âš ï¸ COSTS MONEY. ~6c/book Ã— 25 books â‰ˆ $1.50, estimated from list prices.
npm run backfill:missing-covers -- --remote --llm            # dry run first
npm run backfill:missing-covers -- --remote --llm --commit
```

Yield is **unmeasured** â€” no sweep has been run. Every URL it proposes is fetched
...
| | Item |
|---|---|
| ðŸ”¨ | **Keep GitHub current** â€” the user permits pushing straight to `main` while this site is pre-release, *provided a rollback id is recorded*. Contrast the board game catalog, which has real users now and where changes are "more damning". |
| ðŸ’¤ | **Cross-project TODO page on heygabi.ai** â€” all projects, tagged one/some/all/landing. Explicitly deferred: "we will swap to it later". |
| ðŸ’¤ | Gamefound â€” excluded, no books. |
