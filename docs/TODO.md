...
## ☐ Audio-verdict residue — 1 step left, and it is the OWNER’s: press "Yes this is it" once (~~measure the `rejected` filter~~ MEASURED 2026-09-06 and **SHIPPED to BOTH instances 2026-09-07** — that bullet moved WHOLE to [`DONE.md`](DONE.md) · ~~find padhard’s 27 fold rows~~ FOUND 2026-09-06, below) (from part B, shipped 15:32 — the section is in [`DONE.md`](DONE.md))
...
> ✂️ **2026-09-06 (W13-LIB):** *"UNBUILT — the EBOOK library’s half of
> “say 2”"* moved **WHOLE** to `bookbuddy/audiobook_catalog/docs/TODO.md`, the
> repo that owns both files it names — on the section’s own instruction
> (*"Move it to that repo’s TODO when it is picked up"*). ⚠️ **That tree is
> GITIGNORED**, so the move is on disk there and has no commit; this line is
> the only pointer, and the design notes are not duplicated here.
...
### ⏸️ The AUDIOBOOK catalog's half of "edit any detail + an audit log" — 2026-08-13, still open

**The library half is DONE** — built 2026-08-12/13, live on both instances,
measured 2026-09-07 (`change_log` 1,728 rows on main / 3,978 on padhard; one
authorless work on each). The whole section moved WHOLE to
[`DONE.md`](DONE.md), which carries the measurement and the two deliberate
departures from its own sketch.

What is left is the sibling, and it is the owner's ask verbatim: *"Audiobook
catalog will need this as well."* ⚠️ **A separate item in a separate repo** —
nothing in `library_catalog` touches it. The design of record for BOTH sides is
[`info/edit-and-audit-design.md`](info/edit-and-audit-design.md) §8: the
`change_log` DDL and the key-move rules cross the boundary, the tables never do
(`PLATFORM.md` §2.2). The DDL to copy is
`migrations/0120_change_log_and_authorless.sql`; the sentinel itself
deliberately does **not** cross, because `?unknown` never reaches Firestore by
construction.

⚠️ **Blocked on a prior question, not on effort.** `PLATFORM.md` §2.4 says the
audiobook catalog is pipeline-fed and read-only — *"not a temporary state"* —
and its edit surface today is the `edit_overrides` CLI writing a **git-tracked**
`scripts/catalog_overrides.json`, so git history already is its audit log. The
design's answer (§8) is that it applies this DDL **when it gains a real editor**,
