...
| **The 44 → 46 `SWEEP_BUDGET` raise** | ✅ **OBSERVED ON A LIVE TICK 2026-08-26 22:07 UTC** (it was *"arithmetic, checked by test"* until then). padhard planned 1 book of 90 eligible and spent an estimated **52** on work #541 under `planSweep` rule 4 — the run finished `done` in **48 s** and the invocation was **not** killed, so the worst-case estimate really is a worst case. §8 has the reasoning; `DONE.md` has the tick. |
...
| **Rung 6 (Wikidata) live** | Added 2026-08-25. The SPARQL was verified by hand at `query.wikidata.org` (Way of Kings → Stormlight #1); the rung as wired into the ladder still has only mocked-fetch coverage — ⚠️ **but it now HAS ladder-level coverage at all** (F21's last gap, closed 2026-08-26: a `rung 6 — Wikidata` block, 6 cases). Mocked `fetch` is still not a live run. |
