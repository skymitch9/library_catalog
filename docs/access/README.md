...
> **Last verified (this pass): 2026-09-22** — the `deploy.md` row ONLY, extended
> that day for the 2026-09-22 deploy pair and the new classifier-refusal section,
> against `docs/deploys.log` and the facts measured by the conducting session.
> ⚠️ **No other row was re-checked**, and the `second-instance.md` row was NOT
> changed because that page records no current friend version to update.
> Last verified before that: **2026-09-07** — the `deploy.md` row ONLY, rewritten
> that day for the new `tests.yml` CI gate and checked against the workflow files
> and against run `34156956144`'s own TAP output. ⚠️ No other row was re-checked.
> Last verified before that: **2026-09-05** — the `audiobook-sweep.md` row ONLY,
> rewritten that day for the series-volume half and checked against the file it
> points at and against both live `/api/health` responses. ⚠️ No other row was
> re-checked.
> Last verified before that: **2026-08-14**.
...
| [`deploy.md`](deploy.md) | The short "what order do I do things in" version, plus the Firebase step and the review backfill. 🆕 **2026-09-07 — the CI section**: there are now **two** workflows and only one of them deploys. `tests.yml` runs typecheck + the whole suite on push/PR (no secrets, no Cloudflare, first green run `34156956144`, 2m19s); `deploy.yml` stays **`workflow_dispatch` only** and ⚠️ **must not gain a push trigger** — this Worker has no dev lane. ⚠️ The gate covers **3,026 of 3,027** cases: [KI-21](../KNOWN_ISSUES.md). 🆕 **2026-09-22 — *"`npm run deploy` is refused before check-clean runs — the Claude session's command classifier"***: in auto mode a command that ships to a live domain is refused by the HARNESS, upstream of every guard this repo owns, so you see **no repo output at all** — the table there keeps that apart from a check-clean or deploy-guard refusal, which print their own reasons. Fixed by two `permissions.allow` entries in `~/.claude/settings.json` (⚠️ **not in git** — a rebuilt machine loses them; the rule is owned by `catalog-platform/docs/access/RECOVERY.md`'s machine-state section). ⚠️ `deploy.yml` is **MAIN-only** and therefore not a substitute for the pair. The *Live* table's Worker version is now `a273b877` (2026-09-22 pair; friend `3c684023`) |
...
| [`audiobook-sweep.md`](audiobook-sweep.md) | 🆕 **The audiobook association sweep, now a ROUTE + CRON on both instances (2026-09-06)** — 🔴 **deployed in SHADOW: it computes its whole plan and writes NOTHING, and STEP 11 of the audiobook pipeline is still doing all the writing.** How to read `detail.audiobookSweep` on `/api/health` with no sign-in and what each of the five silences means (`unchanged` / `drift` / `empty snapshot` / `empty-read` / `mode off` are NOT interchangeable); the admin `POST …/audiobooks/sweep` with `{"dryRun":true}` and how to get the owner bearer; the script-vs-route comparison table that is the flip gate (**≥42 shadow ticks with zero divergences**, and a divergence is almost certainly the series-canon skew — the ROUTE is the stale side); the two cron strings that must match in BOTH `[triggers]` blocks; and 🔴 **the rollback, which is one word: `AUDIOBOOK_SWEEP_MODE = "off"`.** ⚠️ Measured 2026-09-06: both migrated, both deployed, both `mode: "shadow"`, plans byte-identical to the script on both — but **no `audiobook_sweep_run` row exists yet on either instance**, so no tick has been observed. Why: [`../info/series-formats-and-audiobooks.md` §4.12](../info/series-formats-and-audiobooks.md). 🆕 **2026-09-05 — ONE TICK, TWO HALVES:** the same invocation now also refreshes `series_volume`/`series_check` from the same fetched CSV (platform inventory §7 row #2), under the **same** `AUDIOBOOK_SWEEP_MODE` — there is deliberately no second switch. `/api/health` gained `detail.audiobookSweep.seriesVolumes`, whose `lastRun` keeps three silences apart (never ran / the on-add hook declining under guard 3 / shadow working). Parity re-measured on both: **MAIN 139 series, 329 statements; padhard 313 series, 453** — and padhard's `series_volume` is **empty**, so nobody was running that script on her instance. 🔴 **CORRECTED: the cron HAS fired** — both instances carry a `04:23` UTC row, `trigger: cron`, `state: shadow`; ~~the ≥42-tick clock is running and now covers BOTH halves.~~ 🔴 **CORRECTED AGAIN 2026-09-06 ~14:20 UTC — a flip to `enforce` was REFUSED on the measurement.** Three run rows exist per instance, **ONE** computed a plan, and **none has ever carried a `seriesVolumes` object**: a `304` returns upstream of that half entirely (`audiobook-sweep-run.ts:430-442`), so the `08:23` and `12:23` ticks filled nothing and were always going to. Only a **200**-tick is evidence, the CSV changes ≈3×/day, and `POST {"dryRun":true}` 304s too — so **the route side of the parity comparison has never been measured in production at all**. Numbers and the three ways to force a tick: that file's §6 and [`../TODO.md`](../TODO.md). Deployed MAIN `6ed4a22b` / friend `c57c5173` |
