...
| [`ebook-retirement.md`](ebook-retirement.md) | 🆕 **Ebook split PHASE 5 — the owner's go/no-go runbook** (2026-09-05). Export → plan → apply → unset `EBOOK_INGEST_TOKEN`, one command each, with the reversal path committed under `docs/archive/`. 🔴 **Built and DRILLED end to end on a throwaway local D1; never applied to either remote.** Carries the two things the design got wrong, both re-measured against production: the *"human-asserted read states must be 0"* precondition is **3** (works 358–360, typed fifteen minutes before the reading that recorded 0), and `--prune --force-prune` — the instrument the design's §6 names — now matches **26 of 127** editions because the 2026-08-20 sweep rewrote `edition.source`. ⚠️ Also: **padhard holds 0 ebook rows** (no-op there but the secret is hers too), and **do not run `backfill:ebooks` afterwards** |
