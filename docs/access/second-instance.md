...
> only MAIN was committed in that mode). ⚠️ **Nothing else on this page was
> re-measured on 2026-09-03**; everything below still carries its 2026-08-25 (or
> older, where noted) measurement.
...
| Sweep "Signed" out of her edition NAMES onto the copies — **clears the whole name** | `node scripts/sweep-signed-editions.mjs --remote --friend` (dry run) → `--commit`. **Applied to her once, 2026-09-03 14:19**: 63 editions normalised, 59 copies flagged, 1 row left for the owner (`docs/TODO.md`). ⚠️ **Never run this mode on MAIN** — its 20 matching names are real vendor prose ("Kickstarter signed paperback"), which migration 0050 says `edition_name` exists to keep. Both instances now match **0**, so a re-run is a no-op |
| The same sweep **keeping the rest of the name** — `--strip-word` | `node scripts/sweep-signed-editions.mjs --remote --strip-word` (dry run; add `--friend` for hers) → `--commit`. Removes only the WORD "signed", leaves `edition_kind` alone, and flags **every** copy of the work — linking one only where it is unambiguous. **Applied to MAIN once, 2026-09-03 14:33**: 20 editions, 16 names rewritten, 4 → NULL, signed copies 25 → 41, 5 rows to link by hand (`docs/TODO.md`). On **padhard it matches 0**, because the default mode had already cleared her names — that is the expected result, not a failure |
