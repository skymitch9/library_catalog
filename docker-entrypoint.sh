...
if [ "${EBOOK_SYNC_DRY_RUN:-1}" = "1" ]; then
    echo "  [DRY RUN] Nothing will be written. Set EBOOK_SYNC_DRY_RUN=0 to publish."
fi

# --- A command passed to `docker run` wins ---
#
# ⚠️ Without this the entrypoint swallows its arguments and starts cron anyway,
# which makes the documented first-run step
#
#     docker compose ... run --rm ebook-sync python scripts/index_cwa_library.py --audit
#
# quietly impossible: you get an hourly cron container instead of a one-shot
# audit, and it sits there Up for as long as you leave it. Found by doing exactly
# that. The config checks above still run first, which is the point of putting
# this here rather than at the top — a one-shot run should fail the same way a
# scheduled one would.
if [ "$#" -gt 0 ]; then
    echo ""
...
env | grep -E '^(LIBRARY_API_|CALIBRE_LIBRARY|SYNC_DATA_DIR|EBOOK_SYNC_DRY_RUN|TZ|PATH|HOME|PYTHONPATH)' > /etc/environment
